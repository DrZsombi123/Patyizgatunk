...
public partial class Victory : Control
{
	...
	public override void _Ready()
	{
		...
		back.Pressed += () => GetTree().ChangeSceneToFile("res://Scenes/TitleScreen.tscn");
		AddChild(back);
		back.GrabFocus();

		var music = new AudioStreamPlayer { Stream = GD.Load<AudioStream>("res://Art/uoai.mp3") };
	...
	private static void AddPart(Node2D dancer, string name, Vector2 position, float scale)
	{
		...
		{
			Texture = GD.Load<Texture2D>($"res://Art/{name}.png"),
			Position = position,
