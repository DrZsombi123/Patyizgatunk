...
public partial class Hotbar : CanvasLayer
{
	...
	public void Restore(string[] items, int[] counts)
	{
		for (int i = 0; i < Slots; i++)
		{
			_items[i] = i < items.Length && items[i] != "" ? items[i] : null;
			_counts[i] = _items[i] != null && i < counts.Length ? counts[i] : 0;
		}

		Refresh();
	}

	// pl. a rendőrség lefoglalja a kocsikulcsot
	...
	private static Texture2D Icon(string item)
	{
		...
		string path = $"res://Art/Items/{item}.png";
